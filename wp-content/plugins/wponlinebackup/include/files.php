...
class WPOnlineBackup_Backup_Files
{
	...
	/*public*/ function WPOnlineBackup_Backup_Files( & $WPOnlineBackup, $db_prefix, $db_force_master = '' )
	{
		...
		$this->exclude_reasons = array(
			...
			WPONLINEBACKUP_FILE_EXCLUDE_CUSTOM	=> __( 'Custom exclude', 'wponlinebackup' ),
		);
	}
	...
	/*public*/ function Initialise( & $bootstrap, & $progress )
	{
		...
		// Resolve the root path - no need to Normalise_Path orig_root since it is only used in the Log_Event below when we fail to resolve it and we Normalise it there
		...
		if ( ( $root = @realpath( $orig_root ) ) === false ) {
			...
			$bootstrap->Log_Event(
				...
				sprintf( __( 'Failed to resolve the WordPress parent folder path, %s: %s.' , 'wponlinebackup' ), $this->_UTF8_Validate( $this->Normalise_Path( $orig_root ) ), $ret )
			);
	...
	/*public*/ function Process_Folder( $scan )
	{
		...
		while ( ( $item = @readdir( $folder ) ) !== false ) {
			...
			if ( is_link( $item_path ) ) {
				...
				$this->bootstrap->Log_Event(
					WPONLINEBACKUP_EVENT_INFORMATION,
					sprintf( __( 'Link %s was skipped: Links are not currently supported' , 'wponlinebackup' ), $this->_UTF8_Validate( $current_folder . $item ) )
				);
				...
				if ( false !== ( $key = array_search( $item_path, $this->job['excludes'] ) ) ) {
					...
					$this->bootstrap->Log_Event(
						...
						sprintf( __( 'Folder %s was skipped: Unknown special characters in the folder name; please remove them to allow backup' , 'wponlinebackup' ), $this->_UTF8_Validate( $current_folder . $item ) )
					);
			...
			// Add the item
				...
				$this->bootstrap->Log_Event(
					WPONLINEBACKUP_EVENT_WARNING,
					sprintf( __( 'File %s was skipped: Unknown special characters in the file name; please remove them to allow backup' , 'wponlinebackup' ), $this->_UTF8_Validate( $current_folder . $item ) )
				);
	...
	/*private*/ function Track_Item( $item, $type, $path = null, $item_id = false )
	{
		...
		if ( $wpdb->query(
			...
			'VALUES ' .
				'(' . WPONLINEBACKUP_BIN_FILESYSTEM . ', ' . $this->folder_id . ', ' . $type . ', \'' . $esc_item . '\', \'' . $esc_item_bin . '\', \'' . $esc_path . '\', ' . $this->progress['activity_id'] . ', ' . $new_counter . ') ' .
			'ON DUPLICATE KEY UPDATE ' .
				'name_bin = IF(activity_id = ' . $this->progress['activity_id'] . ', name_bin, \'' . $esc_item_bin . '\'), ' .
				'path = IF(activity_id = ' . $this->progress['activity_id'] . ', path, \'' . $esc_path . '\'), ' .
			...
			foreach ( $result as $item ) {
				...
				if ( $item['type'] == WPONLINEBACKUP_ITEM_FILE ) {
					...
					if ( is_array( $size ) ) {
						...
						$this->progress['message'] = sprintf( $this->file_taken_msg, $this->_UTF8_Validate( $item_name ) );
						...
						$this->bootstrap->Log_Event(
							WPONLINEBACKUP_EVENT_ERROR,
							sprintf( __( 'File %s was skipped: %s' , 'wponlinebackup' ), $this->_UTF8_Validate( $item_name ), $size )
						);

						$this->progress['message'] = sprintf( $this->file_failed_msg, $this->_UTF8_Validate( $item_name ), $size );
					...
					if ( $success !== true ) {
						...
						$this->bootstrap->Log_Event(
							WPONLINEBACKUP_EVENT_ERROR,
							sprintf( __( 'Status of folder %s was skipped: %s' , 'wponlinebackup' ), $this->_UTF8_Validate( $item_name ), $success )
						);
