...
<?php
	...
	/*private*/ function Get_Help()
	{
		...
		return
			...
			'<p><textarea cols="50" rows="10">' . PHP_EOL .
			...
			'MySQL Client Version: ' . phpversion( 'mysql' ) . PHP_EOL .
			'Mem: ' . ini_get( 'memory_limit' ) . '; Post: ' . ini_get( 'post_max_size' ) . '; Upload: ' . ini_get( 'upload_max_filesize' ) . '; Timeout: ' . ini_get( 'max_execution_time' ) . PHP_EOL .
...
<?php
	...
	/*public*/ function Print_Page()
	{
		...
		$links = array(
			...
			'help'		=> _x( 'Help & Support', 'Plugin section', 'wponlinebackup' ),
		);
		...
		if ( is_null( $activity ) ) {
			...
			switch ( $activity['comp'] ) {
				...
				case WPONLINEBACKUP_COMP_MAINTENANCE:
					$message = array( 'exclamation.png', 'A00', _x( 'Maintenance', 'Activity Log', 'wponlinebackup' ) );
					break;
		...
		if (
			...
			wp_localize_script(
				...
				array(
					...
					'Events_URL'		=> 'tools.php?page=' . urlencode( WPONLINEBACKUP_FILE ),
...
<?php
	...
	/*private*/ function Prepare_Decrypt()
	{
		...
		$this->WPOnlineBackup->Enable_Error_Handling();
...
<?php
	...
	{
		add_meta_box( 'wponlinebackuphelpsupport', _x( 'Help & Support', 'Help subsection title', 'wponlinebackup' ), array( & $this, 'Print_Help' ), 'wponlinebackuphelp', 'normal' );
	}
